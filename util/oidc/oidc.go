...
// If querying the UserInfo endpoint fails, we return an error, requiring callers to force relogin
...
func (a *ClientApp) AddGroupsFromUserInfo(claims jwt.Claims, SessionManagerClaimsIssuer string) (jwt.MapClaims, error) {
	...
	if iss != SessionManagerClaimsIssuer && a.settings.UserInfoGroupsEnabled() && a.settings.UserInfoPath() != "" {
		...
		if unauthorized {
			return groupClaims, fmt.Errorf("error while quering userinfo endpoint: %v", err)
		}
		if err != nil {
			return groupClaims, fmt.Errorf("error fetching user info endpoint: %v", err)
		}
		if groupClaims["sub"] != userInfo["sub"] {
			return groupClaims, fmt.Error(codes.Unknown, "subject of claims from user info endpoint didn't match subject of idToken, see https://openid.net/specs/openid-connect-core-1_0.html#UserInfo")
		}
